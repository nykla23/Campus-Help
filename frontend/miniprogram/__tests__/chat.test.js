...
jest.mock('../utils/api', () => ({
  ...
  downloadAvatar: jest.fn().mockResolvedValue('data:image/png;base64,mockavatar'),
...
describe('聊天页面 Chat', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    api.sendMsgApi.mockImplementation(() => Promise.resolve({ code: 200, message: 'ok', data: { id: 123 } }));
    ...
    api.getTaskDetail.mockImplementation(() => Promise.resolve({ code: 200, data: {
      id: 1, title: '任务', publisher: { id: '1', nickname: '发布者' }, acceptor: null
    } }));
  });
  ...
  describe('Mock API 测试', () => {
    ...
    test('sendMsg 发送失败/异常显示错误', async () => {
      ...
      page.setData({ inputMsg: '消息', taskId: '1', targetId: '2', myId: 10 });
      await page.sendMsg();
    ...
    test('loadChat 加载成功解析列表', async () => {
      api.getChatDetail.mockResolvedValueOnce({
        code: 200, data: [{ id: 1, from_id: 10, content: '你好', created_at: '2025-06-15T10:00:00' }]
      });
